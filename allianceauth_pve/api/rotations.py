...
from allianceauth_pve.utils import ensure_rotation_presets_applied
...
def create_rotation(request, data: NewRotationSchema):  # noqa: ARG001
...
def close_rotation(request, rotation_id: int, data: CloseRotationSchema):
    ...
    with transaction.atomic():
        ...
        item_ids = set(
            EntryLootItem.objects.filter(entry__rotation=rotation)
    ...
    project_summaries_cache_key = ROTATION_PROJECT_SUMMARY_CACHE_KEY.format(
    ...
    cache.delete_many(
        ...
        for user_id in EntryCharacter.objects.filter(entry__rotation=rotation)
...
@router.get(
    "/{int:rotation_id}/summary/",
    response={200: list[RotationSummarySchema], 404: None},
)
def get_rotation_summary(request, rotation_id: int):  # noqa: ARG001
    try:
        rotation = Rotation.objects.get(pk=rotation_id)
    except Rotation.DoesNotExist:
        return 404, None
...
@router.get(
    "/{int:rotation_id}/project_summaries/",
    response={200: list[RotationProjectSummarySchema], 404: None},
)
def get_rotation_project_summaries(request, rotation_id: int):  # noqa: ARG001
    try:
        rotation = Rotation.objects.get(pk=rotation_id)
    except Rotation.DoesNotExist:
        return 404, None
...
@router.get(
    "/{int:rotation_id}/buttons/", response={200: list[PveButtonSchema], 404: None}
)
def get_rotation_buttons(request, rotation_id: int):  # noqa: ARG001
    try:
        rotation = Rotation.objects.get(pk=rotation_id)
    except Rotation.DoesNotExist:
        return 404, None
...
@router.get(
    "/{int:rotation_id}/items/",
    response={200: list[ExtendedEntryItemSchema], 404: None},
)
def get_rotation_items(request, rotation_id: int):  # noqa: ARG001
    try:
        rotation = Rotation.objects.get(pk=rotation_id)
    except Rotation.DoesNotExist:
        return 404, None
